...
int main( int argc, char **argv, char **env )
{
    ...
    gtk_window_set_default_size( GTK_WINDOW (window[OUTER]), config.i[WINDOW_WIDTH][SETTING], config.i[WINDOW_HEIGHT][SETTING] );
    table[OUTER] = gtk_vbox_new( FALSE, 0 );
    gtk_container_add( GTK_CONTAINER (window[OUTER]), table[OUTER] );

    if( !config.b[HIDE_MENU_BAR][SETTING] ) {
        /* Setup Menu */
...
//     gtk_clist_set_column_justification( GTK_CLIST (buglist), 1, GTK_JUSTIFY_CENTER );
    ...
    if( !config.b[HIDE_BUTTONS][SETTING] ) {
        /* Setup Button Table */
